...
// Update function prototype to include verbose parameter
int search_eggs(int field[N][N], int start_row, int end_row, bool verbose);
...
int main(int argc, char *argv[]) {
    ...
    int provided; // For MPI thread support level
    ...
    if (rank == 0) {
        ...
        printf("Hybrid parallelization: %d MPI processes, each with up to %d OpenMP threads\n", 
        ...
        eggs_found = search_eggs(field, start_row, end_row, verbose);
        ...
        printf("Search completed in %f seconds with %d MPI processes and %d OpenMP threads per process\n", 
...
// Search for eggs in a portion of the field (hybrid parallelization)
int search_eggs(int field[N][N], int start_row, int end_row, bool verbose) {
    int eggs_found = 0;
    
    // Use OpenMP to parallelize the search within each MPI process
    #pragma omp parallel
    {
        int thread_id = omp_get_thread_num();
        int thread_count = omp_get_num_threads();
        
        // Only the first thread in each process prints this info, and only in verbose mode
        #pragma omp master
        {
            if (verbose) {
                printf("Process at rows %d-%d using %d OpenMP threads\n", 
                       start_row, end_row-1, thread_count);
            }
        }
        
        // Parallelize the row search across threads
        #pragma omp for reduction(+:eggs_found)
        ...
        // Only print per-thread statistics in verbose mode
        if (verbose) {
            #pragma omp critical
            {
                printf("Thread %d found eggs contributing to the process total\n", thread_id);
            }
        }
    }
    
    // Only print the final summary in verbose mode
    if (verbose) {
        printf("Process searched rows %d-%d and found %d eggs total\n", 
